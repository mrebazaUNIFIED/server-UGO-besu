...
import USFCIListener from './listeners/USFCIListener.js';
import USFCIBridgeOutHandler from './handlers/USFCIBridgeOutHandler.js';
import USFCIBridgeInHandler from './handlers/USFCIBridgeInHandler.js';
  ...
  try {
    ...
    switch (event.type) {
      ...
      case 'USFCI_BRIDGE_OUT':
        ...
        break;
      case 'USFCI_BRIDGE_IN':
        handler = new USFCIBridgeInHandler();
        break;
      default:
...
async function initialize() {
  try {
    ...
    usfciListener = new USFCIListener(eventQueue);
    await usfciListener.start();
...
async function shutdown() {
  ...
  try {
    ...
    if (usfciListener) usfciListener.stop();
    // Close API server
    if (apiServer) {
      apiServer.close();
    }
