    ...
    UUPSUpgradeable
{
    ...
    event TokensBridgedToAvalanche(
        address indexed sender,
    ...
    function bridgeToAvalanche(
        ...
        // 1. Quemamos los tokens en la red privada (Trujillo)
        _burn(msg.sender, amount);
        accounts[msg.sender].lastActivity = block.timestamp;

        // 2. Registramos el burn para auditoría interna
        burnRecords.push(
            BurnRecord({
                burnerAddress: msg.sender,
                burnerMspId: accounts[msg.sender].mspId,
                amount: amount,
                reason: "Bridge to Avalanche",
                timestamp: block.timestamp
            })
        );

        // 3. Emitimos el evento que el Relayer está escuchando
        emit TokensBridgedToAvalanche(
            msg.sender,
            target,
            amount,
            block.timestamp
